...
<html>
        <head>
            ...
            <title>⚠️ZELENE MILITARY BERMUDE⚠️</title>
            <script>
...
<span style="font-size:7vh;cursor:pointer;float:right;margin:3vh" id="open_nav_button" onclick="openNav()">&#9776;</span>
  ...
  let containerHTML = "";
  for(let i = 1; i <= j; i++){
    containerHTML += "<div class='mySlides'><div class='numbertext'>"+i+" / "+j+"</div><img src='/uploads/87_"+i+".jpg' style='height:470px;max-width:100%;object-fit: cover;'></div>";
  }
  ...
  let rowHTML = "";
  for(let i = 1; i <= j; i++){
    rowHTML += "<div class='column'><img class='demo cursor' src='/uploads/87_"+i+".jpg' style='width:100%;height:125px;object-fit: cover;' onclick='currentSlide("+i+")' alt='The Woods'></div>";
  }
...
<div id="article-info-part">
    <h1>⚠️ZELENE MILITARY BERMUDE⚠️</h1>
    <h2>54.99 KM</h2>
    <hr>
    <p>⚠️MILITARY BERMUDE⚠️ 

📃 Opis:
Materijal: 100% pamuk
Veličine: S- XXXXL
Dostupne boje: zelena i crna
Karakteristike: veliki broj džepova, praktične, prijatne za nošenje i imaju mesto za amblem. 
