...
<html>
        <head>
            ...
            <title>⚠️AIRSOFT ŠLEMOVI⚠️</title>
            <script>
...
<span style="font-size:7vh;cursor:pointer;float:right;margin:3vh" id="open_nav_button" onclick="openNav()">&#9776;</span>
  ...
  let containerHTML = "";
  for(let i = 1; i <= j; i++){
    containerHTML += "<div class='mySlides'><div class='numbertext'>"+i+" / "+j+"</div><img src='/uploads/70_"+i+".jpg' style='height:470px;max-width:100%;object-fit: cover;'></div>";
  }
  ...
  let rowHTML = "";
  for(let i = 1; i <= j; i++){
    rowHTML += "<div class='column'><img class='demo cursor' src='/uploads/70_"+i+".jpg' style='width:100%;height:125px;object-fit: cover;' onclick='currentSlide("+i+")' alt='The Woods'></div>";
  }
...
<div id="article-info-part">
    <h1>⚠️AIRSOFT ŠLEMOVI⚠️</h1>
    <h2>69.99 KM</h2>
    <hr>
    <p>⚠️AIRSOFT ŠLEMOVI⚠️ 

📃 Opis:
Materijal: tvrda plastika
Dimenzije: univerzalna
Dostupne boje: crna i zelena
Karakteristike: izdržljivost, praktičnost, šlem poseduje šine za dodatke, čičak za oznake i kamuflažu izuzetno prijatan za nošenje 
