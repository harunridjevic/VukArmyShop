...
<html>
        <head>
            ...
            <title>⚠️LOVAČKO TAKTIČKI NOŽ⚠️</title>
            <script>
...
<span style="font-size:7vh;cursor:pointer;float:right;margin:3vh" id="open_nav_button" onclick="openNav()">&#9776;</span>
  ...
  let containerHTML = "";
  for(let i = 1; i <= j; i++){
    containerHTML += "<div class='mySlides'><div class='numbertext'>"+i+" / "+j+"</div><img src='/uploads/118_"+i+".jpg' onclick='openFullScreen()' style='height:470px;max-width:100%;object-fit: cover;'></div>";
  }
  ...
  let rowHTML = "";
  for(let i = 1; i <= j; i++){
    rowHTML += "<div class='column'><img class='demo cursor' src='/uploads/118_"+i+".jpg' style='width:100%;height:125px;object-fit: cover;' onclick='currentSlide("+i+")' alt='The Woods'></div>";
  }
...
<div id="fullscreenimage" style="transition: 0.3s; display: none; background-color: rgba(0, 0, 0, 0.3); width: 100%; height: 100%; position: fixed; top: 0; left: 0; z-index: 2000;">
  <div style="background-color: rgba(0, 0, 0, 0.8); width: 100%; height: 10%; position: fixed; top: 0; left: 0; z-index: 2000;">
    <div id="fullscreenx" style=" font-size: 5vh; padding: 1.5vh 8vh 1.5vh 0; float: right; transition: 0.3s;" onclick="closeFullScreen()">x</div>
  </div>
  <img id="fullscreenimage_image" src="112_1.jpg" style="margin: 5% auto 0 auto; display: block; height: 90%;">
  <a class="prev" style="position: fixed; left: 0; top: 55%;" onclick="plusSlides(-1)">❮</a>
  <a class="next" style="position: fixed; right: 0; top: 55%;" onclick="plusSlides(1)">❯</a>
</div>
...
<div id="article-info-part">
    <h1>⚠️LOVAČKO TAKTIČKI NOŽ⚠️</h1>
    <h2>79.99 KM</h2>
    <hr>
    <p>⚠️LOVAČKO TAKTIČKI NOŽ⚠️ 

📃 Opis:
Dimenzije: ukupna dužina 29cm dužina sečiva 16,5cm
Dostupne boje: crna i zelena
Karakteristike: lagan teži svega 250g, izdržljiv, crna zaštita sečiva, nož dolazi u kompletu sa futrolom koja se kači na opasač 
...
function showSlides(n) {
  ...
  document.getElementById("fullscreenimage_image").src= dots[slideIndex-1].src;
}
</script>
